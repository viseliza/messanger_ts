...
export const load: PageServerLoad = async () => {
	...
	const replacement: string = await getReplacemnt(group, "src/xlsx/30.10.2023.doc");
	return {
		schedule,
		replacement
	}
...
function getSchedule(group: string, path: string): string {
	...
	for (let row = 7; row < Object.keys(data).length - 1; row++) {
		let time: string = data[row][column - 1];
		let replacement: string = data[row][column];
		let day_of_the_weak: string = data[row][column - 2];
