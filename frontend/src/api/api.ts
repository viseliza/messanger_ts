...
export class AppAPI {
    ...
    async checkUser(data: User, first_name?: string, last_name?: string, father_name?: string) {
        let user;
        try {
            user = await this.callApi('user', { name: data.login });
        } catch {
            user = await this.callApi("auth", { body: { login: data.login, password: data.password } });
            
            const group_name = (await (await fetch(`https://people.novsu.ru/NovSUScheduleService/ScheduleProxy?uid=${data.login}`))
            .text()).split('src')[1].split('=')[4].split('&')[0];
            const group = await this.callApi('group', { name: group_name });
            
            const profile: Profile = await this.callApi('profile/auth', { 
                body: {
                    first_name,
                    last_name,
                    father_name,
